...
namespace NUMC.Design.Controls
{
    public class TreeView : ScrollView
    {
        ...
        private TreeNode _anchoredNodeStart;
        private TreeNode _anchoredNodeEnd;
        ...
        private TreeNode _provisionalNode;
        private TreeNode _dropNode;
        private bool _provisionalDragging;
        private List<TreeNode> _dragNodes;
        ...
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public ObservableCollection<TreeNode> SelectedNodes { get; }
        ...
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public IComparer<TreeNode> TreeViewNodeSorter { get; set; }
        ...
        private void ChildNodes_ItemsAdded(object sender, ObservableListModified<TreeNode> e)
        {
            foreach (var node in e.Items)
                HookNodeEvents(node);
        ...
        private void ChildNodes_ItemsRemoved(object sender, ObservableListModified<TreeNode> e)
        {
            foreach (var node in e.Items)
            {
        ...
        private void HookNodeEvents(TreeNode node)
        {
        ...
        private void UnhookNodeEvents(TreeNode node)
        {
            node.Nodes.ItemsAdded -= ChildNodes_ItemsAdded;
            node.Nodes.ItemsRemoved -= ChildNodes_ItemsRemoved;
        ...
        private void UpdateNode(TreeNode node, ref TreeNode prevNode, int indent, ref int yOffset,
        ...
        private void UpdateNodeBounds(TreeNode node, int yOffset, int indent)
        {
        ...
        private void LoadIcons()
        {
            ...
            _nodeClosed = Render.ColorChange(Icons.node_closed_empty, _styles.ScrollView.Color);
            _nodeClosedHover = Render.ColorChange(Icons.node_closed_empty, _styles.Control.ColorSelectionBackgroundColor);
            _nodeClosedHoverSelected = Render.ColorChange(Icons.node_closed_full, _styles.ScrollView.Color);
            _nodeOpen = Render.ColorChange(Icons.node_open, _styles.ScrollView.Color);
            _nodeOpenHover = Render.ColorChange(Icons.node_open, _styles.Control.ColorSelectionBackgroundColor);
            _nodeOpenHoverSelected = Render.ColorChange(Icons.node_open_empty, _styles.ScrollView.Color);
        }
        ...
        private void NodeMouseLeave(TreeNode node)
        {
        ...
        private void CheckNodeHover(TreeNode node, Point location)
        {
            if (IsDragging)
            {
        ...
        private void CheckNodeClick(TreeNode node, Point location, MouseButtons button)
        {
            var rect = GetNodeFullRowArea(node);
        ...
        private void CheckNodeDoubleClick(TreeNode node, Point location)
        {
            var rect = GetNodeFullRowArea(node);
            if (rect.Contains(location))
            {
        ...
        public void SelectNodes(TreeNode startNode, TreeNode endNode)
        {
        ...
        private void SelectAnchoredRange(TreeNode node)
        {
            _anchoredNodeEnd = node;
        ...
        public Rectangle GetNodeFullRowArea(TreeNode node)
        {
        ...
        private void SortChildNodes(TreeNode node)
        {
        ...
        private TreeNode FindNode(TreeNode parentNode, string path, bool recursive = true)
        {
        ...
        protected virtual bool ForceDropToParent(TreeNode node)
        {
            return false;
        }

        protected virtual bool CanMoveNodes(List<TreeNode> dragNodes, TreeNode dropNode, bool isMoving = false)
        {
            if (dropNode == null)
                return false;
            ...
            foreach (var node in dragNodes)
            {
                if (node == dropNode)
                    //if (isMoving)
                    //    MessageBox.Show($"Cannot move {node.Text}. The destination folder is the same as the source folder.",
                    //        Application.ProductName, System.Windows.Forms.MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                while (parentNode != null)
                {
                    if (node == parentNode)
                        //if (isMoving)
                        //    MessageBox.Show($"Cannot move {node.Text}. The destination folder is a subfolder of the source folder.",
                        //        Application.ProductName, System.Windows.Forms.MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        protected virtual void MoveNodes(List<TreeNode> dragNodes, TreeNode dropNode) { }

        protected virtual void NodesMoved(List<TreeNode> nodesMoved) { }
        ...
        private void DrawNode(TreeNode node, Graphics g)
        {
            ...
            // 1. Draw background
            var bgColor = node.Odd ? _styles.ScrollView.HeaderBackgroundColor : _styles.ScrollView.BackgroundColor;
            ...
            // 4. Draw text
            using (var b = new SolidBrush(_styles.ScrollView.Color))
            {
