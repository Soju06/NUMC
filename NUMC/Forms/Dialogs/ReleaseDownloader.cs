...
namespace NUMC.Forms.Dialogs {
    public partial class ReleaseDownloader : Dialog {
        ...
        private void DocumentCompleted(string url) {
            if (url != _releaseUrl) return;
            else _releaseUrl = "Completed";
            documentTimer.Stop();
        ...
        private void ReleaseDownloader_Load(object sender, EventArgs e) => 
            Navigate();
        ...
        private void DocumentTimer_Tick(object sender, EventArgs e) {
            if (TryCount >= TimeOut) SetTimeOut();
            else if (_releaseUrl == contentBox?.Url?.ToString() 
                && contentBox?.ReadyState == WebBrowserReadyState.Complete)
